...
# (v0.17.0 upstream design, adopted in place of our older version-substring
# allowlists.) Claude 4.6 replaced budget-based extended thinking with
# *adaptive* thinking, and 4.7 additionally forbids the manual ``thinking``
# block entirely and drops temperature/top_p/top_k.  Newer Claude releases
# (4.8, and named models like claude-fable-5) follow the same modern contract
# but share no common version substring, so an allowlist of version numbers
# ("4.6", "4.7", …) goes stale the moment a model ships without a recognized
# number and silently routes it down the legacy manual-thinking path.
#
...
# thinking.  This mirrors the "default to newest" output-limit design (future
# models are unlikely to regress to the older contract), so each new Claude
