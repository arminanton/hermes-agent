...
def _maybe_wrap_anthropic(
    ...
    _force_copilot_claude = (
        base_url_host_matches(base_url, "githubcopilot.com")
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    if (not or_key
            ...
            and (base_url_hostname(custom_base) == "api.openai.com"
                 or base_url_host_matches(custom_base, "githubcopilot.com"))):
    ...
    *,
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            # Fallback order (#26882, #26803, #28xxx-main-fallback-chain, plus
            # local recovery hardening):
            #   * auto users iterate the auto-detection chain in order:
            #       1. user-configured fallback_chain (per-task)
            #       2. top-level main fallback_providers/fallback_model
            #       3. built-in auxiliary payment/discovery chain
            #     skipping any fallback labels that also fail with recoverable
            #     capacity errors (for example depleted OpenRouter 402 before a
            #     working Nous lane).
            ...
            while True:
                ...
                if is_auto:
                    ...
                    if fb_client is None and "main-fallback-chain" not in fallback_skip_labels:
                        fb_client, fb_model, fb_label = _try_main_fallback_chain(
                            task, resolved_provider or "auto", reason=reason)
                        fallback_skip_labels.add("main-fallback-chain")
    ...
    *,
    ...
    try:
        ...
        if should_fallback and (is_auto or is_capacity_error):
            ...
            # Fallback order mirrors sync call_llm (#26882, #26803,
            # #28xxx-main-fallback-chain), including local recovery hardening for
            # fallback candidates that themselves fail with recoverable capacity
            # errors. Auto ladder: configured-chain → main-fallback-chain →
            # payment/discovery; explicit: configured-chain → main agent model.
            fallback_skip_labels: set[str] = set()
            while True:
                fb_client, fb_model, fb_label = (None, None, "")
                if is_auto:
