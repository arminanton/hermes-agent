...
def _content_policy_blocked_result(
    ...
    *,
    ...
    The ``refused`` flag lets callers distinguish a deterministic safety
    refusal from a generic failure (e.g. to skip retry/escalation logic that
    only makes sense for transient errors).
    """
    return {
...
def run_conversation(
    ...
    # Commentary deduplication spans continuations and tool calls within one
    # user turn, but the same wording may be valid again on the next turn.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if _pre_api_steer:
            ...
            if not _injected:
                ...
                # autopilot turn that sits inside one multi-minute tool, could
        ...
        # Overlay: configurable via compression.max_attempts (agent_init sets
        # agent.max_compression_attempts); upstream hardcodes 3. Keep ours.
            ...
            try:
                ...
                if finish_reason == "content_filter":
                    ...
                    return _content_policy_blocked_result(
                        ...
                        error_detail=_refusal_text
                        or "safety refusal: model declined (content_filter)",
                    )
        ...
        try:
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                if (
                    ...
                    current_visible = agent._interim_assistant_visible_text(interim_msg)
                    if last_visible or current_visible:
                        same_visible_output = last_visible == current_visible
                    else:
                        same_visible_output = (
