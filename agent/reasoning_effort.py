...
Ported from NousResearch/hermes-agent main (commit f7d90c9410). Kept in sync
with our live Copilot findings (2026-08-20): ``CODEX_GPT56_EFFORTS`` includes
``max`` (gpt-5.6-only), ``CODEX_LEGACY_EFFORTS`` stops at ``xhigh`` (gpt-5.4/5.5
reject ``max`` AND ``minimal``), grok-4.6 adds ``xhigh`` over grok-4.5.
"""
...
#: xAI Responses, Grok 4.6+ accepts xhigh; older Grok tops out at high. Same
#: sets apply to grok on the GitHub Copilot /responses route (live-verified
#: 2026-08-20: grok-4.6 accepts through xhigh, grok-4.5 low/medium/high).
XAI_GROK46_EFFORTS: tuple[str, ...] = ("low", "medium", "high", "xhigh")
XAI_LEGACY_EFFORTS: tuple[str, ...] = ("low", "medium", "high")


def grok_supported_efforts(model: Optional[str]) -> tuple[str, ...]:
    """Supported effort set for a Grok model slug (xAI or Copilot /responses)."""
    m = (model or "").strip().lower().split("/")[-1]
    if m.startswith("grok-4.6"):
        return XAI_GROK46_EFFORTS
    if m.startswith("grok-4.5"):
        return XAI_LEGACY_EFFORTS
    # Unknown grok: no effort dial rather than a 400 on an unsupported level.
    return ()
...
#: Anthropic adaptive-thinking (output_config.effort) on Claude 4.6+: the
#: Copilot proxy accepts low/medium/high/xhigh/max, rejects none/minimal
#: (live-verified 2026-08-20 against claude-opus-4.8). 4.6 family also lacks
#: xhigh (accepts low/medium/high/max); read the live catalog for the exact
#: per-model set when available.
ANTHROPIC_ADAPTIVE_EFFORTS: tuple[str, ...] = ("low", "medium", "high", "xhigh", "max")
ANTHROPIC_ADAPTIVE_46_EFFORTS: tuple[str, ...] = ("low", "medium", "high", "max")
