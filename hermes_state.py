...
# pysqlite3 connections). A single `except sqlite3.OperationalError` would
...
# Same cross-driver concern for DatabaseError: a malformed-schema error may be
# raised (or, in tests, constructed) by EITHER the active driver or stdlib
# sqlite3, and the two driver's DatabaseError classes are distinct. A single
# `isinstance(exc, sqlite3.DatabaseError)` would miss the other driver's class.
_DatabaseErrors: tuple = (sqlite3.DatabaseError,)
try:
    import sqlite3 as _stdlib_sqlite3  # noqa: F811  - stdlib alias
    if _stdlib_sqlite3.DatabaseError is not sqlite3.DatabaseError:
        _DatabaseErrors = (sqlite3.DatabaseError, _stdlib_sqlite3.DatabaseError)
except ImportError:  # pragma: no cover - stdlib always available
    pass
...
def is_malformed_db_error(exc: BaseException) -> bool:
    ...
    only recovery path.
    """
    if not isinstance(exc, _DatabaseErrors):
        return False
