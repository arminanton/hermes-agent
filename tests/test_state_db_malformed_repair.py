...
# SessionDB raises the *active* sqlite driver's DatabaseError (pysqlite3 in
# production, stdlib when HERMES_SQLITE_DRIVER=stdlib). The two driver's
# DatabaseError classes are distinct, so a bare stdlib ``sqlite3.DatabaseError``
# in ``pytest.raises`` would miss a pysqlite3-raised error. Catch the driver's
# actual class.
_ActiveDatabaseError = hermes_state.sqlite3.DatabaseError
...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    with pytest.raises(_ActiveDatabaseError):
