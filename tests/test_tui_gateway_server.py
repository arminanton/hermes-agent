...
def test_notification_poller_delivers_completion(monkeypatch):
    ...
    # The poller only auto-STARTS an agent turn when notify_autodispatch
    # policy allows it (default "autopilot"): an idle plain chat must not
    # "respond by itself" when a background job finishes. Engage autopilot so
    # this test exercises the auto-dispatch path it is written to verify.
    sess = _session(agent=_Agent(), autopilot=True)
...
def test_notification_poller_requeues_when_busy(monkeypatch):
    ...
    # Busy-requeue only applies on the auto-dispatch path, which the
    # notify_autodispatch policy gates to autopilot sessions by default.
    # Engage autopilot so the poller reaches the running-busy check it
    # is written to verify; running=True then forces the requeue.
    sess = _session(running=True, autopilot=True)  # agent is busy
...
def test_notification_poller_emits_distinct_watch_matches_once(monkeypatch):
    ...
    # notify_autodispatch gates auto-START of agent turns to autopilot
    # sessions by default; engage it so each distinct watch match drives a
    # _run_prompt_submit call as this test verifies.
    sess = _session(autopilot=True)
