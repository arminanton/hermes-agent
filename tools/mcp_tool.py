            ...
            while True:
                ...
                if self.session:
                    ...
                    if self._rpc_lock.locked() or self._inflight_tasks:
                        continue
                    try:
                        # Take the SAME _rpc_lock tool calls use so a call
                        # starting between the guard above and here can't
                        # overlap the keepalive on the single JSON-RPC stream
                        # (council-hang fix). Probe body is upstream's richer
                        # ping-first / list_tools-fallback _keepalive_probe.
                        async with self._rpc_lock:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            _inflight = getattr(server, "_inflight_tasks", None)
            if task is not None and _inflight is not None:
                _inflight.add(task)
            try:
                async with server._rpc_lock:
                    ...
                    if hasattr(server, "_pending_call_context"):
