...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if raw:
            ...
            # ``_detect_file_drop`` and ``_resolve_attachment_path`` already
            # return None unless the path resolves to a real, existing local
            # file (see cli._resolve_attachment_path's exists()/is_file() gate),
            # so a non-None ``candidate`` is always a real on-disk image. The
            # classic remote-backend case (desktop sent a client-local path the
            # gateway can't see) makes both helpers return None, falling through
            # to the data_url branch below. Trust the helpers; an extra exists()
            # re-check here is redundant and breaks callers that legitimately
            # supply a resolved path.
